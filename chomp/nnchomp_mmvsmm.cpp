...
int minmax_rec(Board *now,int turn,int left_depth){
  ...
  if(turn==MY_TURN){
    ...
    for(int y=0;y<HEIGHT;y++){
      for(int x=0;x<WIDTH;x++){
        ...
        if(next){
          int score=minmax_rec(next,(turn+1)%2,left_depth-1);
...
Board *minmax(Board *now,int left_depth){
  ...
  for(int y=0;y<HEIGHT;y++){
    for(int x=0;x<WIDTH;x++){
      Board *next=bite(now,x,y);
      if(next){
        int score=minmax_rec(next,YOUR_TURN,left_depth-1);
        if(score>max_score){
          if(ret)free(ret);
          ret=next;
          max_score=score;
